...
class AppointmentPolicy
{
    /*
        ...
        - read_appointments: Permite ver sus citas asignadas (para DOCTOR)
        - manage_appointments: Permite crear, editar y eliminar citas (para ASSISTANT y ADMIN)
    */

    public function viewAny(User $user): bool
    {
        return $user->hasAnyPermission(['read_all_appointments', 'read_appointments']);
    }

    public function view(User $user, Appointment $appointment): bool
    {
        if ($user->hasPermissionTo('read_all_appointments')) return true;
        return $user->hasPermissionTo('read_appointments') && $user->doctor?->id === $appointment->doctor_id;
    }
    ...
    public function viewDoctorAppointments(User $user, int $doctorId): bool
    {
        ...
        return $user->hasPermissionTo('read_appointments') &&
            $user->doctor?->id === $doctorId;
    }

    public function viewPatientAppointments(User $user): bool
    {
        return $user->hasPermissionTo('read_all_appointments');
    }
}
